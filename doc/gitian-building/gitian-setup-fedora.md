...
The build script we'll be using is contained in the VIP github repository ([contrib/gitian-build.py](https://github.com/vip-project/vip/blob/master/contrib/gitian-build.py)). Since this is a completely fresh environment, we haven't yet cloned the VIP repository and will need to fetch this script with the following commands:

```bash
curl -L -O https://raw.githubusercontent.com/VIP-Project/VIP/master/contrib/gitian-build.py
...
- Clone the gitian-builder, gitian.sigs, vip-detached-sigs, and vip GitHub repos.
