...
*Setup instructions for a gitian build of VIP Core using a VM or physical system.*
...
to the VIP GitHub release page.
...
- [Building VIP Core](#building-vip-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/vip-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/vip-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/vip-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/vip-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/vip-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/vip-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/vip-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@vip.org) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/vip-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/vip-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/vip-osx-*-build.assert
```
