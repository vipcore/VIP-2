...
An extended description of this large work can be found in the PR [here](https://github.com/VIP-Project/VIP/pull/1327).
...
Automatic zVIP backup has been disabled. Thus, the following configuration options have been removed  (either as entries in the vip.conf file or as startup flags):
- `autozvipbackup`
- `backupzvip`
- `zvipbackuppath`
...
The default value for the stake-split threshold has been lowered from 2000 VIP, down  to 500 VIP.


Dependencies
------------

The minimum required version of QT has been increased from 5.0 to 5.5.1 (the [depends system](https://github.com/vip-project/vip/blob/master/depends/README.md) provides 5.9.7)
...
- "moneysupply" and "zvipSupply" attributes in `getblock` output are removed.
...
- The output of `getstakingstatus` was reworked. It now shows the following information:
  ```
  {
     ...
     "staking_enabled": true|false,      (boolean) whether staking is enabled/disabled in vip.conf
     "coldstaking_enabled": true|false,  (boolean) whether cold-staking is enabled/disabled in vip.conf
     "haveconnections": true|false,      (boolean) whether network connections are present
     "mnsync": true|false,               (boolean) whether masternode data is synced
     "walletunlocked": true|false,       (boolean) whether the wallet is unlocked
     "stakeablecoins": n,                (numeric) number of stakeable UTXOs
     "stakingbalance": d,                (numeric) VIP value of the stakeable coins (minus reserve balance, if any)
...
The p2p alert system has been removed in [PR #1372](https://github.com/VIP-Project/VIP/pull/1372) and the 'alert' message is no longer supported.

### GUI

**Keyboard navigation**: dialogs can now be accepted with the `ENTER` (`RETURN`) key, and dismissed with the `ESC` key ([#1392](https://github.com/VIP-Project/VIP/pull/1392)).
...
Addresses in the dropdown of the "Send Transaction" and "Send Delegation" widgets are now automatically sorted by label with ascending order ([#1393](https://github.com/VIP-Project/VIP/pull/1393)).


**Custom Fee**: The custom fee selected when sending a transaction is now saved in the wallet database and persisted across multiple sends and wallet's restarts ([#1406](https://github.com/VIP-Project/VIP/pull/1406)).


**Include delegations in send**: The send and cold-staking page present a checkbox ([#1391](https://github.com/VIP-Project/VIP/pull/1391)) to make the automatic input selection algorithm include delegated (P2CS) utxos if needed. The option is unchecked by default.


**Staking Charts**: can now be hidden at startup (with a flag `--hidecharts`) or at runtime with a checkbox in settings --> options --> display ([PR #1475](https://github.com/VIP-Project/VIP/pull/1475)).
...
__Context Lock/Unlock__ [[PR #1387](https://github.com/VIP-Project/VIP/pull/1387)]:<br>
...
- The `-reservebalance` configuration/startup option has been removed ([PR #1373](https://github.com/VIP-Project/VIP/pull/1373)).
