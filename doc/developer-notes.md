...
- [Development guidelines](#development-guidelines)
    - [General VIP Core](#general-vip-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
VIP Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
VIP Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/vip-project/vip/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_vip
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_vip --log_level=test_suite
$ valgrind -v --leak-check=full src/vipd -printtoconsole
```
...
# A coverage report will now be accessible at `./test_vip.coverage/index.html`.
```
...
- BitcoinMiner : Generates VIPs (if wallet is enabled).
...
be sent upstream but bugfixes may also be prudent to PR against VIP Core so that they can be integrated
...
file descriptors it will cause problems with VIP's `select()` loop, because
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `vip-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `vip-cli` and the GUI debug console use this table to determine how to
