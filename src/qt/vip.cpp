...
#include "qt/vip/splash.h"
#include "qt/vip/welcomecontentwidget.h"
...
public:
    ...
    /// Get window identifier of QMainWindow (VIPGUI)
...
{
    window = new VIPGUI(networkStyle, 0);

    pollShutdownTimer = new QTimer(window);
    connect(pollShutdownTimer, &QTimer::timeout, window, &VIPGUI::detectShutdown);
...
void BitcoinApplication::startThread()
{
    ...
    connect(window, &VIPGUI::requestedRestart, executor, &BitcoinCore::restart);
...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if (retval) {
        ...
        if (pwalletMain) {
            ...
            window->addWallet(VIPGUI::DEFAULT_WALLET, walletModel);
            window->setCurrentWallet(VIPGUI::DEFAULT_WALLET);
        ...
        //connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &VIPGUI::handlePaymentRequest);
        connect(window, &VIPGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
{
    QMessageBox::critical(0, "Runaway exception", VIPGUI::tr("A fatal error occurred. VIP can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(1);
}
