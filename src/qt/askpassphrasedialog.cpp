...
void AskPassphraseDialog::accept()
{
    ...
    case Mode::Encrypt: {
        ...
        bool ret = openStandardDialog(
                ...
                " <b>" + tr("LOSE ALL OF YOUR VIP") + "</b>!<br><br>" + tr("Are you sure you wish to encrypt your wallet?"),
                tr("ENCRYPT"), tr("CANCEL")
        );
        if (ret) {
            if (newpass1 == newpass2) {
                newpassCache = newpass1;
                VIPGUI* window = static_cast<VIPGUI*>(parentWidget());
...
{
    VIPGUI* gui = static_cast<VIPGUI*>(parentWidget());
...
{
    hide();
    static_cast<VIPGUI*>(parentWidget())->showHide(true);
    openStandardDialog(
            tr("Wallet encrypted"),
            "<qt>" +
