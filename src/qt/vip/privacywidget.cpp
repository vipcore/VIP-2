...
#include "qt/vip/privacywidget.h"
#include "qt/vip/forms/ui_privacywidget.h"
#include "qt/vip/qtutils.h"
#include "guiutil.h"
#include "qt/vip/txviewholder.h"
...
PrivacyWidget::PrivacyWidget(VIPGUI* parent) :
    PWidget(parent),
    ui(new Ui::PrivacyWidget)
{
    ...
    /* Subtitle */
    ui->labelSubtitle1->setText(tr("Minting zVIP anonymizes your VIP by removing any\ntransaction history, making transactions untraceable "));
    setCssSubtitleScreen(ui->labelSubtitle1);

    ui->labelSubtitle2->setText(tr("Mint new zVIP or convert back to VIP"));
    ...
    ui->lineEditAmount->setPlaceholderText("0.00 VIP ");
    ...
    ui->labelValueDenom1->setText("0x1 = 0 zVIP");
    ...
    ui->labelValueDenom5->setText("0x5 = 0 zVIP");
    ...
    ui->labelValueDenom10->setText("0x10 = 0 zVIP");
    ...
    ui->labelValueDenom50->setText("0x50 = 0 zVIP");
    ...
    ui->labelValueDenom100->setText("0x100 = 0 zVIP");
    ...
    ui->labelValueDenom500->setText("0x500 = 0 zVIP");
    ...
    ui->labelValueDenom1000->setText("0x1000 = 0 zVIP");
    ...
    ui->labelValueDenom5000->setText("0x5000 = 0 zVIP");
    setCssProperty(ui->labelValueDenom5000, "text-body2");

    ui->layoutDenom->setVisible(false);

    // List
    ui->labelListHistory->setText(tr("Last zVIP Movements"));
    ...
    // Only Convert to VIP enabled.
    ui->containerViewPrivacyChecks->setVisible(false);
    onMintSelected(false);

    ui->btnTotalzVIP->setTitleClassAndText("btn-title-grey", "Total 0 zVIP");
    ui->btnTotalzVIP->setSubTitleClassAndText("text-subtitle", "Show denominations of zVIP owned.");
    ui->btnTotalzVIP->setRightIconClass("ic-arrow");

    ui->btnCoinControl->setTitleClassAndText("btn-title-grey", "Coin Control");
    ui->btnCoinControl->setSubTitleClassAndText("text-subtitle", "Select VIP outputs to mint into zVIP.");
    ...
    ui->btnResetZerocoin->setTitleClassAndText("btn-title-grey", "Reset Spent zVIP");
    ui->btnResetZerocoin->setSubTitleClassAndText("text-subtitle", "Reset zerocoin database.");

    connect(ui->btnTotalzVIP, &OptionButton::clicked, this, &PrivacyWidget::onTotalZvipClicked);
...
void PrivacyWidget::onMintSelected(bool isMint)
{
    QString btnText;
    if (isMint) {
        btnText = tr("Mint zVIP");
        ui->btnCoinControl->setVisible(true);
        ui->labelSubtitleAmount->setText(tr("Enter amount of VIP to mint into zVIP"));
    } else {
        btnText = tr("Convert back to VIP");
        ui->btnCoinControl->setVisible(false);
        ui->labelSubtitleAmount->setText(tr("Enter amount of zVIP to convert back into VIP"));
    }
...
void PrivacyWidget::onTotalZvipClicked()
{
    bool isVisible = ui->layoutDenom->isVisible();
    if (!isVisible) {
        ui->layoutDenom->setVisible(true);
        ui->btnTotalzVIP->setRightIconClass("btn-dropdown", true);
    } else {
        ui->layoutDenom->setVisible(false);
        ui->btnTotalzVIP->setRightIconClass("ic-arrow", true);
    }
}
...
void PrivacyWidget::onSendClicked()
{
    ...
    if (sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        warn(tr("Zerocoin"), tr("zVIP is currently undergoing maintenance"));
        return;
    }
    ...
    if (!ctx.isValid()) {
        inform(tr("You need to unlock the wallet to be able to %1 zVIP").arg(isConvert ? tr("convert") : tr("mint")));
        return;
    }
...
void PrivacyWidget::mint(CAmount value)
{
    ...
    if (!walletModel->mintCoins(value, CoinControlDialog::coinControl, strError)) {
        ...
        // Mint succeed
        inform(tr("zVIP minted successfully"));
        // clear
...
void PrivacyWidget::spend(CAmount value)
{
    ...
    if (!walletModel->convertBackZvip(
            value,
            selectedMints,
            receipt
    )) {
        ...
        // Spend succeed
        inform(tr("zVIP converted back to VIP"));
        // clear
        ui->lineEditAmount->clear();
    }
}
...
void PrivacyWidget::updateDenomsSupply()
{
    ...
    ui->btnTotalzVIP->setTitleText(tr("Total %1").arg(GUIUtil::formatBalance(matureZerocoinBalance, nDisplayUnit, true)));
}
