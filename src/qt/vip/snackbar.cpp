...
#include "qt/vip/forms/ui_snackbar.h"
#include "qt/vip/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(VIPGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &VIPGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
