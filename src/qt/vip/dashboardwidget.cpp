...
#include "qt/vip/dashboardwidget.h"
#include "qt/vip/forms/ui_dashboardwidget.h"
#include "qt/vip/sendconfirmdialog.h"
#include "qt/vip/txrow.h"
...
DashboardWidget::DashboardWidget(VIPGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    // Staking Information
    ui->labelMessage->setText(tr("Amount of VIP and zVIP staked."));
    setCssSubtitleScreen(ui->labelMessage);
    setCssProperty(ui->labelSquarePiv, "square-chart-vip");
    setCssProperty(ui->labelSquarezPiv, "square-chart-zvip");
    setCssProperty(ui->labelPiv, "text-chart-vip");
    setCssProperty(ui->labelZvip, "text-chart-zvip");
    ...
    ui->labelAmountZvip->setText("0 zVIP");
    ui->labelAmountPiv->setText("0 VIP");
    setCssProperty(ui->labelAmountPiv, "text-stake-vip-disable");
    setCssProperty(ui->labelAmountZvip, "text-stake-zvip-disable");
    ...
    ui->btnHowTo->setText(tr("How to get VIP or zVIP"));
...
bool hasCharts = false;
    ...
    if (window)
        connect(window, &VIPGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif
...
void DashboardWidget::loadWalletModel()
{
    if (walletModel && walletModel->getOptionsModel()) {
        ...
        connect(ui->pushImgEmpty, &QPushButton::clicked, window, &VIPGUI::openFAQ);
        connect(ui->btnHowTo, &QPushButton::clicked, window, &VIPGUI::openFAQ);
...
const QMap<int, std::pair<qint64, qint64>> DashboardWidget::getAmountBy()
{
    ...
    for (int i = 0; i < size; ++i) {
        ...
        bool isPiv = modelIndex.data(TransactionTableModel::TypeRole).toInt() != TransactionRecord::StakeZVIP;
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    chartData->amountsByCache = getAmountBy(); // pair VIP, zVIP
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            vip = (pair.first != 0) ? pair.first / 100000000 : 0;
            zvip = (pair.second != 0) ? pair.second / 100000000 : 0;
            chartData->totalPiv += pair.first;
            chartData->totalZvip += pair.second;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesPiv.append(vip);
        chartData->valueszPiv.append(zvip);

        int max = std::max(vip, zvip);
...
void DashboardWidget::onChartRefreshed()
{
    ...
    // init sets
    set0 = new QBarSet("VIP");
    set1 = new QBarSet("zVIP");
    ...
    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalPiv > 0 || chartData->totalZvip > 0) {
        setCssProperty(ui->labelAmountPiv, "text-stake-vip");
        setCssProperty(ui->labelAmountZvip, "text-stake-zvip");
    } else {
        setCssProperty(ui->labelAmountPiv, "text-stake-vip-disable");
        setCssProperty(ui->labelAmountZvip, "text-stake-zvip-disable");
    }
    forceUpdateStyle({ui->labelAmountPiv, ui->labelAmountZvip});
    ui->labelAmountPiv->setText(GUIUtil::formatBalance(chartData->totalPiv, nDisplayUnit));
    ui->labelAmountZvip->setText(GUIUtil::formatBalance(chartData->totalZvip, nDisplayUnit, true));
