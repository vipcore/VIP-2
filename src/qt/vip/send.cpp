...
#include "qt/vip/send.h"
#include "qt/vip/forms/ui_send.h"
#include "qt/vip/addnewcontactdialog.h"
#include "qt/vip/qtutils.h"
#include "qt/vip/sendchangeaddressdialog.h"
#include "qt/vip/optionbutton.h"
#include "qt/vip/sendconfirmdialog.h"
#include "qt/vip/myaddressrow.h"
#include "qt/vip/guitransactionsutils.h"
#include "clientmodel.h"
#include "optionsmodel.h"
#include "addresstablemodel.h"
#include "coincontrol.h"
#include "script/standard.h"
#include "zvip/deterministicmint.h"
#include "openuridialog.h"
#include "zvipcontroldialog.h"

SendWidget::SendWidget(VIPGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    ...
    btnContacts(new QPushButton())
{
    ...
    /* Button Group */
    ui->pushLeft->setText("VIP");
    setCssProperty(ui->pushLeft, "btn-check-left");
    ui->pushLeft->setChecked(true);
    ui->pushRight->setText("zVIP");
    setCssProperty(ui->pushRight, "btn-check-right");

    /* Subtitle */
    ui->labelSubtitle1->setText(tr("You can transfer public coins (VIP) or private coins (zVIP)"));
    ...
    /* Address */
    ui->labelSubtitleAddress->setText(tr("VIP address or contact label"));
    ...
    ui->labelAmountSend->setText("0.00 VIP");
    ...
    setCssProperty(coinIcon, "coin-icon-vip");
    ...
    // Connect
    connect(ui->pushLeft, &QPushButton::clicked, [this](){onVIPSelected(true);});
    connect(ui->pushRight,  &QPushButton::clicked, [this](){onVIPSelected(false);});
...
void SendWidget::refreshView()
{
    ...
    ui->pushButtonSave->setText(isChecked ? tr("Send VIP") : tr("Send zVIP"));
...
void SendWidget::refreshAmounts()
{
    ...
    bool isZvip = ui->pushRight->isChecked();
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();

    ui->labelAmountSend->setText(GUIUtil::formatBalance(total, nDisplayUnit, isZvip));
...
void SendWidget::showHideCheckBoxDelegations()
{
    ...
    // coincontrol is not selected, and we are trying to spend VIP (not zVIP)
    const bool isZvip = ui->pushRight->isChecked();
    const bool isCControl = CoinControlDialog::coinControl->HasSelected();
    const bool hasDel = cachedDelegatedBalance > 0;

    const bool showCheckBox = !isZvip && !isCControl && hasDel;
    ...
    if (showCheckBox)
        ui->checkBoxDelegations->setToolTip(
                tr("Possibly spend coins delegated for cold-staking (currently available: %1").arg(
                        GUIUtil::formatBalance(cachedDelegatedBalance, nDisplayUnit, isZvip))
        );
}
...
void SendWidget::onSendClicked()
{
    ...
    if ((sendPiv) ? send(recipients) : sendZvip(recipients)) {
...
bool SendWidget::sendZvip(QList<SendCoinsRecipient> recipients)
{
    if (!walletModel || !walletModel->getOptionsModel())
        return false;

    if (sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        Q_EMIT message(tr("Spend Zerocoin"), tr("zVIP is currently undergoing maintenance."), CClientUIInterface::MSG_ERROR);
        return false;
    }
    ...
    if (walletModel->sendZvip(
        ...
        inform(tr("zVIP transaction sent!"));
        ...
        if (receipt.GetStatus() == ZVIP_SPEND_V1_SEC_LEVEL) {
            body = tr("Version 1 zVIP require a security level of 100 to successfully spend.");
        } else {
            int nNeededSpends = receipt.GetNeededSpends(); // Number of spends we would need for this transaction
            const int nMaxSpends = Params().GetConsensus().ZC_MaxSpendsPerTx; // Maximum possible spends for one zVIP transaction
        ...
        Q_EMIT message("zVIP transaction failed", body, CClientUIInterface::MSG_ERROR);
        return false;
    }
}
...
void SendWidget::onCoinControlClicked()
{
    if (isVIP) {
        if (walletModel->getBalance() > 0) {
            ...
            inform(tr("You don't have any VIP to select."));
        }
    } else {
        if (walletModel->getZerocoinBalance() > 0) {
            ...
            inform(tr("You don't have any zVIP in your balance to select."));
        }
    }
}
...
void SendWidget::onVIPSelected(bool _isVIP)
{
    isVIP = _isVIP;
    setCssProperty(coinIcon, _isVIP ? "coin-icon-vip" : "coin-icon-zvip");
    refreshView();
...
void SendWidget::onContactMultiClicked()
{
    if (focusedEntry) {
        ...
        CBitcoinAddress vipAdd = CBitcoinAddress(address.toStdString());
        if (walletModel->isMine(vipAdd)) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(vipAdd.Get(), dialog->getLabel().toStdString(),
                    AddressBook::AddressBookPurpose::SEND)) {
