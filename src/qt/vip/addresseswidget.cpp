...
#include "qt/vip/forms/ui_addresseswidget.h"
#include "qt/vip/addresslabelrow.h"
#include "qt/vip/addnewaddressdialog.h"
...
AddressesWidget::AddressesWidget(VIPGUI* parent) :
    PWidget(parent),
    ui(new Ui::AddressesWidget)
{
    ...
    // Address
    ui->labelAddress->setText(tr("Enter VIP address"));
...
void AddressesWidget::onStoreContactClicked(){
    if (walletModel) {
        ...
        CBitcoinAddress vipAdd = CBitcoinAddress(address.toUtf8().constData());
        ...
        if (walletModel->updateAddressBookLabels(vipAdd.Get(), label.toUtf8().constData(),
                vipAdd.IsStakingAddress() ? AddressBook::AddressBookPurpose::COLD_STAKING_SEND : AddressBook::AddressBookPurpose::SEND)
                ) {
