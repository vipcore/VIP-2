...
CzVIPWallet::CzVIPWallet(CWallet* parent)
{
    this->wallet = parent;
...
void CzVIPWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (std::pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (wallet->zvipTracker->HasPubcoinHash(pMint.first)) {
...
bool CzVIPWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    wallet->zvipTracker->Add(dMint, true);
...
void CzVIPWallet::GenerateDeterministicZVIP(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzVIPWallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzVIPWallet::CheckSeed(const CDeterministicMint& dMint)
{
