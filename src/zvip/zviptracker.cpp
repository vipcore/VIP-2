...
#include "zvip/zvipwallet.h"


CzVIPTracker::CzVIPTracker(CWallet* parent)
{
    this->wallet = parent;
...
void CzVIPTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzVIPWallet* zVIPWallet)
{
    bool iszVIPWalletInitialized = (nullptr != zVIPWallet);
    CMintMeta meta;
    ...
    if (!iszVIPWalletInitialized)
        zVIPWallet = new CzVIPWallet(wallet);
...
std::set<CMintMeta> CzVIPTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(wallet->strWalletFile);
    if (fUpdateStatus) {
        ...
        LogPrint(BCLog::LEGACYZC, "%s: added %d dzvip from DB\n", __func__, listDeterministicDB.size());
    }
