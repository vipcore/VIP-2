...
bool CheckZerocoinSpend(const CTransaction& tx, bool fVerifySignature, CValidationState& state, bool fFakeSerialAttack)
{
    ...
    for (const CTxIn& txin : tx.vin) {
        ...
        if (isPublicSpend) {
            ...
            if (!ZVIPModule::parseCoinSpend(txin, tx, prevOut, publicSpend)){
        ...
        if (isPublicSpend) {
            ...
            if (!ZVIPModule::validateInput(txin, prevOut, tx, ret)){
    ...
    if (nHeight >= consensus.height_start_ZC_SerialsV2) {
        try {
            if (!spend->HasValidSignature())
                return error("%s: V2 zVIP spend does not have a valid signature\n", __func__);
...
bool RecalculateVIPSupply(int nHeightStart, bool fSkipZvip)
{
    ...
    while (true) {
        ...
        // Rewrite zvip supply too
        if (!fSkipZvip && pindex->nHeight >= consensus.height_start_ZC) {
