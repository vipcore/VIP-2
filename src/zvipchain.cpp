...
std::string ReindexZerocoinDB()
{
    ...
    while (pindex) {
        ...
        // update supply
        UpdateZVIPSupplyConnect(block, pindex, true);
...
{
    CDataStream serializedCoinSpend = ZVIPModule::ScriptSigToSerializedSpend(txin.scriptSig);
...
bool UpdateZVIPSupplyConnect(const CBlock& block, CBlockIndex* pindex, bool fJustCheck)
{
    AssertLockHeld(cs_main);
    ...
    //Add mints to zVIP supply (mints are forever disabled after last checkpoint)
    ...
    // A one-time event where only the zVIP supply was off (due to serial duplication off-chain on main net)
...
bool UpdateZVIPSupplyDisconnect(const CBlock& block, CBlockIndex* pindex)
{
    AssertLockHeld(cs_main);
    ...
    // Re-add spends to zVIP supply
    std::list<libzerocoin::CoinDenomination> listDenomsSpent = ZerocoinSpendListFromBlock(block, true);
    for (const libzerocoin::CoinDenomination& denom : listDenomsSpent) {
        mapZerocoinSupply.at(denom)++;
    }

    // Remove mints from zVIP supply (mints are forever disabled after last checkpoint)
    if (pindex->nHeight < consensus.height_last_ZC_AccumCheckpoint) {
        std::list<CZerocoinMint> listMints;
        std::set<uint256> setAddedToWallet;
        BlockToZerocoinMintList(block, listMints, true);
